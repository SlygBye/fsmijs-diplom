...
## создать файл .env.db и определить в нём следующие переменные окружения :
```bash
Имя переменной                     Описание значения переменной
MONGO_INITDB_ROOT_USERNAME         Имя пользователя для подключения к БД 
MONGO_INITDB_ROOT_PASSWORD         Пароль пользователя для подключения к БД,   
